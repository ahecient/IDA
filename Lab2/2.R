...
new_df <- data.frame(word = character(),x = integer(),y = integer(), stringsAsFactors = FALSE)
  ...
  for (j in 1:length(test_words)){
    ...
    for (l in 1:str_which(test_words, test_words[j]))
    {
      if (!(str_which(test_words, test_words[l]) == str_which(test_words, test_words[j]))){
        summ <- summ + str_length(test_words[l])
      }
    }
...
new_df <- subset(new_df, number!=1)
new_df$number <- NULL
new_df_tree <- data.frame(word = new_df$word, levels = 0)

for(k in 1:(nrow(new_df_tree)-2)){
  dm <- numeric(0) 
  ...
  tria_dm <- dm 
  tria_dm[upper.tri(tria_dm, diag = TRUE)] <-NA 
  print(tria_dm)
  min_dm <-which(tria_dm == min(tria_dm, na.rm = TRUE), arr.ind = TRUE)
  min_r <- rownames(tria_dm)[min_dm[,1]] 
